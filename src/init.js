...
var notification = {
	...
	local_min:function(_seconds){
	
		badge_number++;
		var d = new Date();
		var d2 = new Date(d);
		d2.setMinutes(d.getMinutes()+_seconds);
		//d2.setMinutes(d2.getMinutes()-120);
		//console.log(d2);
		//d = d.getTime();
        //d = d.getTime() + (_seconds*30000); 
		
		//d = new Date(d);
		
		//d = d.setDate(d.getDate()+_seconds);
		
		//d =d.getTime()+_seconds;
        
		//alert(d.toString('yyyy-mm-dd'));
		//console.log(d.toString('yyyy-mm-dd'));
		plugins.localNotification.add({
			date: d2,
			repeat:'daily',
			message: 'A new fly is available',
			hasAction: true,
			badge: badge_number,
			//id: '1',
			id:_seconds,
